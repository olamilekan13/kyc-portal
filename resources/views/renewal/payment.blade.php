    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white rounded-2xl shadow-xl p-6 sm:p-8 mb-6 border border-gray-100">
            <div class="flex items-start space-x-4">
                ...
                <div class="flex-1">
                    <h1 class="text-3xl font-bold text-gray-900 mb-2">Complete Renewal Payment</h1>
                    <p class="text-gray-600 leading-relaxed">Make your payment to renew your partnership.</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-xl border border-gray-100 p-6 sm:p-8 mb-6">
            ...
            <div class="space-y-4 mb-6">
                <div class="flex justify-between items-center pb-3 border-b border-gray-200">
                    <div>
                        <p class="font-semibold text-gray-900">New Partnership Model</p>
                        <p class="text-sm text-gray-600">{{ $renewalData['partnership_model_name'] }}</p>
                    </div>
                    <p class="text-lg font-bold text-gray-900">₦{{ number_format($renewalData['partnership_model_price'], 2) }}</p>
                </div>

                <div class="flex justify-between items-center pb-3 border-b border-gray-200">
                    <div>
                        <p class="font-semibold text-gray-900">Duration</p>
                        <p class="text-sm text-gray-600">Partnership validity period</p>
                    </div>
                    <p class="text-lg font-bold text-gray-900">
                        @if($renewalData['duration_months'] == 1)
                            1 Month
                        @elseif($renewalData['duration_months'] < 12)
                            {{ $renewalData['duration_months'] }} Months
                        @elseif($renewalData['duration_months'] == 12)
                            1 Year
                        @else
                            {{ $renewalData['duration_months'] }} Months
                ...
                <div class="flex justify-between items-center pt-3">
                    ...
                    <p class="text-3xl font-bold text-blue-600">₦{{ number_format($renewalData['partnership_model_price'], 2) }}</p>
                </div>
            </div>
        </div>
        ...
        <!-- Back Button -->
        <div class="mb-6">
            <a href="{{ route('renewal.show', ['token' => $finalOnboarding->renewal_token]) }}" class="inline-flex items-center text-blue-600 hover:text-blue-800 font-semibold transition-colors">
        ...
        <!-- Payment Methods -->
        <div class="bg-white rounded-2xl shadow-xl border border-gray-100 p-6 sm:p-8" x-data="{ activeTab: '{{ $renewalData['payment_method'] === 'paystack' ? 'online' : 'bank' }}' }">
            ...
            <div x-show="activeTab === 'bank'" x-cloak>
                ...
                <form action="{{ route('renewal.bank-transfer', ['token' => $finalOnboarding->renewal_token]) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="space-y-4">
                        <div>
            ...
            <div x-show="activeTab === 'online'" x-cloak>
                ...
                <div class="space-y-4">
                    <button
                        onclick="payWithPaystack({{ $renewalData['partnership_model_price'] }})"
                    ...
                    >
                        <div class="flex justify-between items-center">
                            <div>
                                <p class="text-lg font-bold">Pay Now</p>
                                <p class="text-sm opacity-90">Renewal Fee</p>
                            </div>
                            <p class="text-2xl font-bold">₦{{ number_format($renewalData['partnership_model_price'], 2) }}</p>
                        </div>
                    </button>
                </div>
            </div>
...
<script>
    function payWithPaystack(amount) {
        const handler = PaystackPop.setup({
            ...
            email: '{{ $finalOnboarding->partner_email ?? 'customer@example.com' }}',
            amount: amount * 100, // Amount in kobo
            currency: 'NGN',
            ref: 'RNW-' + Math.floor((Math.random() * 1000000000) + 1),
            metadata: {
                renewal: true,
                final_onboarding_id: {{ $finalOnboarding->id }}
            },
            callback: function(response) {
                window.location = '{{ route('renewal.paystack-callback', ['token' => $finalOnboarding->renewal_token]) }}?reference=' + response.reference;
            },
            onClose: function() {
